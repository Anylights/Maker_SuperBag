...
1. 设计瓦片清单（按 tile-design-rules.md 的分类体系）
2. 先生成 1 张锚点瓦片（如 grass_base1），确认风格满意
3. 用锚点作为 reference_image + 固定 seed，分批生成其余瓦片
4. 每批 2-4 张（避免超时），方向性瓦片的 prompt 必须明确"哪半/哪角是什么"
5. 全部生成后复制到目标目录，去掉时间戳后缀
